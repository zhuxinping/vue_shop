<template>
    <div>
                ...
                <li v-for="(item,index) in cartList">
                  <div class="cart-tab-1">
                    <div class="cart-item-check">
                      <a @click="editCart('checked',item)" href="javascipt:;" class="checkbox-btn item-check-btn" :class="{'check':item.checked=='1'}">
                    ...
                    <div class="item-quantity">
                      <div class="select-self select-self-open">
                        <div class="select-self-area">
                          ...
                          <span class="select-ipt" v-if="item.productNum>=0">{{item.productNum}}</span>
            ...
            <div class="cart-foot-inner">
              <div class="cart-foot-l">
                <div class="item-all-check">
                  <a @click="toggleCheckAll"  href="javascipt:;">
      ...
      <Modal :mdShow="modalConFirm" v-on:close="closeModal">
        <p slot="message">你确认你要删除此条数据嘛?</p>
        <div slot="btnGroup">
          <a href="javascript:void(0)" @click="delCart" class="btn btn--m">确认</a>
          <a href="javascript:void(0)" @click="modalConFirm=false" class="btn btn--m">关闭</a>
        </div>
      </Modal>
      <MFooter></MFooter>
    </div>
...
<script>
  ...
  import {CartList,DelCart,EditCart,CheckAll} from '../api/index'
    ...
    export default {
      ...
      filters:{
        ...
        methods: {
          ...
          async getCartList(){
           let {result,status}= await CartList();
          // console.log(result);
           if(status=="0"){
          ...
          //删除数据
          async delCart(){
            let {status} =await DelCart(this.productId);
            if(status=="0"){
              this.modalConFirm=false;
              this.getCartList();
              let reduceCount=0;
              this.cartList.forEach(item=>{
                if(item.productId==this.productId){
                  reduceCount=item.productNum;
                }
              });
              //this.cartList=result;
              this.$store.commit("updateCartCount",-reduceCount);
            }
          },
          async editCart(flag,item){
            /*if(num<0&&item.productNum<=0){
              //如果数目小等于0就不在请求减去商品
              item.productNum=0;
              return;
            }*/
            ...
            let {status}= await EditCart(item);
            //console.log(status);
            if(status=="0"){
              let num=0;
              if(flag=="add"){
          ...
          async toggleCheckAll(){
            ...
            let {status}= await CheckAll(flag);
            //console.log(status)
            if(status=="0"){
              console.log("success");
            }
