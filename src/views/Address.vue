<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="next-btn-wrap">
              <router-link :to="{path:'/orderConfirm',query:{'addressId':selectedAddId}}" class="btn btn--m btn--red">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <Modal :mdShow="isMdShow" @close="closeModal">
        ...
        <div slot="btnGroup">
          <a href="javascript:;" class="btn btn--m" @click="sureDelAddress">确认</a>
          <a href="javascript:;" @click="isMdShow=false" class="btn btn--m">取消</a>
        </div>
      </Modal>
      <MFooter></MFooter>
    </div>
...
<script>
  ...
  import {AddressList,SetAddress,DelAddress} from '../api/index'
  export default{
      data(){
          return{
            addressList:[],
    ...
    methods:{
      ...
      async sureDelAddress(){
        this.isMdShow=false;
        let {status,result}=await DelAddress(this.addressId);
        if(status=="0"){
          //console.log('suceess');
          this.getAddressList();//也可以直接前端更改UI数据
        }
      },
        closeModal(){
          this.isMdShow=false;
...
let {status,result}=await SetAddress(addressId);
   if(status=="0"){
     //console.log('suceess');
     this.getAddressList();//也可以直接前端更改UI数据
   }
      ...
      async getAddressList(){
        let {status,result}=await AddressList();
       // console.log(status);
        if(status=='0'){
          //console.log(result);
