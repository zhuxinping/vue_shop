...
<script>
  ...
  import {CartList,PayMent} from '../api/index'
    export default {
        data() {
            return {
              goodsList:[],
      ...
      filters:{
        ...
        methods: {
          async getCartData(){
            let {status,result}=await CartList();
            if(status=='0'){
        ...
        async payMent(){
           ...
           //console.log(addressId);
            let {status,result}=await PayMent(
              {
                addressId:addressId,
            ...
            if(status=='0'){
              //console.log('order');
              this.$router.push(`/orderSuccess?orderId=${result.orderId}`)
            }
          }
        },
        computed: {},
