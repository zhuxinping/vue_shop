<template>
  <header class="header">
    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          ...
          <span class="navbar-link" v-cloak v-if="nickName">{{nickName}}</span>
          <a @click="loginModalFlag=true" href="javascript:void(0)" v-if="!nickName" class="navbar-link">Login</a>
          <a @click="logout" href="javascript:void(0)" class="navbar-link" v-if="nickName">Logout</a>
      ...
      <div class="md-modal-inner">
        ...
        <div class="md-content">
          <div class="confirm-tips">
            <div class="error-wrap" v-show="errorTip" style="color:#d31723;">用户名或者密码错误</div>
            <ul>
              ...
              <li class="regi_form_input noMargin">
                <i class="icon IconPwd"></i> <input @keyup.enter="login" v-model="userPwd" type="password" tabindex="2" name="password" placeholder="Password" class="regi_login_input regi_login_input_left login-input-no input_text">
              </li>
            </ul>
          </div>
    ...
    <div @click.stop="loginModalFlag=false"  v-if="loginModalFlag"  class="md-overlay"></div>
  </header>
</template>

<script>
  import '../assets/css/login.css';
  import {Login,LogOut,CheckLogin,GetCartCount} from '../api/index';
  import {mapState} from "vuex";
    export default {
      ...
      mounted(){
        ...
        methods: {
        async checkLog(){
          let {status,result} = await CheckLogin();

          if(status=='0'){
            //this.nickName=result;把登录信息保存到vuex里面的state
        ...
        async login(){
          if(!this.userName||!this.userPwd){
           ...
           let {status,result} = await Login({userName:this.userName,userPwd:this.userPwd});
          // console.log(result);
           if(status=="0"){
             this.errorTip=false;
         ...
         async logout(){
          ...
          async getCartCount(){
           let {result,status}= await GetCartCount();
           if(status=='0'){
             this.$store.commit("initCartCount",result);
           }
          }
        }
    }
